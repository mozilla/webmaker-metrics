...
function productUVsWeb (callback) {
  'use strict';
...
function productUVsApp (callback) {
  'use strict';
  var args = {metricName: 'AppUVs', days: 30};
  getProductSnapshotValue(args, function (err, result) {
    if (err) {
      console.error(err);
      return callback(err);
...
/**
 * NOTE: In the future, if you update the data format of this reporting, send a
 * note to exec-dashboards@mozilla.org so they can update their code too.
 */
