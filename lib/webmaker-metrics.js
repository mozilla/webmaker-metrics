...
function crunchForDaysForRID (periodEndDate, periodLengthDays, rid, callback) {
  ...
  async.parallelLimit({
    ...
    // USERS REPEAT
    usersRepeat: function(callback){
      integrationDb.usersRepeat(qryObj, function (err, res) {
        if (err) {
          console.error(err);
        }
        toSave.usersRepeat = res;
        callback();
      });
    },

    // LEARNERS NEW
    learnersNew: function(callback){
      integrationDb.learnersNew(qryObj, function (err, res) {
        if (err) {
          console.error(err);
        }
        toSave.learnersNew = res;
        callback();
      });
    },

    // LEARNERS REPEAT
    learnersRepeat: function(callback){
      integrationDb.learnersRepeat(qryObj, function (err, res) {
        if (err) {
          console.error(err);
        }
        toSave.learnersRepeat = res;
        callback();
      });
    },
    ...
    // MVC BADGES REPEAT
    mvcBadgeRepeat: function(callback){
      integrationDb.mvcBadgeRepeat(qryObj, function (err, res) {
        if (err) {
          console.error(err);
        }
        toSave.mvcBadgeRepeat = res;
        callback();
      });
    },
    ...
    // EVENT HOST REPEAT
    eventHostsRepeat: function(callback){
      integrationDb.eventHostsRepeat(qryObj, function (err, res) {
        if (err) {
          console.error(err);
        }
        toSave.eventHostsRepeat = res;
        callback();
      });
    },
    ...
    // MENTOR - REPEAT
    mentorsRepeat: function(callback){
      integrationDb.mentorsRepeat(qryObj, function (err, res) {
        if (err) {
          console.error(err);
        }
        toSave.mentorsRepeat = res;
        callback();
      });
    },
    ...
    // SUPER MENTOR - REPEAT
    superMentorsRepeat: function(callback){
      integrationDb.superMentorsRepeat(qryObj, function (err, res) {
        if (err) {
          console.error(err);
        }
        toSave.superMentorsRepeat = res;
        callback();
      });
    },
    ...
    // MAKER - REPEAT
    makersRepeat: function(callback){
      integrationDb.makersRepeat(qryObj, function (err, res) {
        if (err) {
          console.error(err);
        }
        toSave.makersRepeat = res;
        callback();
      });
    }
  },
...
function crunchForGivenDays (periodEndDate, periodLengthDays, callback) {
  ...
  // for each one, save a set of numbers
  var rid = 'makerparty2014_snippet';
  crunchForDaysForRID(periodEndDate, periodLengthDays, rid, function (err) {
    ...
    // run this twice while testing:
    crunchForDaysForRID(periodEndDate, periodLengthDays, null, function (err) {
      if (err) {
        console.error(err);
      }
      return callback();
