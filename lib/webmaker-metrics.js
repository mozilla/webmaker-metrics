...
function runCrunchForProductFunnel (snapshotDate, callback) {
  ...
  // create a product funnel snapshop obj
  var toSave = {
    snapshotDate: snapshotDate
  };
  ...
  async.parallelLimit({
    ...
    AU7dayRetention: function(callback){
      // How many users who joined 7-14 days prior were active > 7 days
      // after their join date
      integrationDb.productAU7dayRetention(qryObj, function (err, res) {
        if (err) {
          console.error(err);
        }
        toSave.AU7dayRetention = res;
        callback();
      });
    },
  ...
  function(err) {
    ...
    // calculations
    toSave.UVtoAU = toSave.AUs / toSave.UVs;

    // Save the object we've built
    db.ProductFunnelSnapshot.create(toSave).error(function (err) {
      // Error
      console.error(err);
      return callback(err);
      ...
      // Success
      console.log('Saved Product Funnel Snapshot');
      return callback();
    });
  });
