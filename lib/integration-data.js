 ...
 * {obj}   args (args.periodEndDate, args.periodLengthDays OR args.periodStartDate)
 * @param  {Function} callback
 */
function countUsersActive (args, callback) {
  'use strict';
  var startDateInclusive;
  var endDateInclusive = aggregateEndDate(args);

  if (args.periodLengthDays) {
    startDateInclusive = aggregateStartDate(args);
  }

  if (args.periodStartDate) {
    startDateInclusive = args.periodStartDate;
  }
...
/**
 * The number of new users in a given time period, who Opt in to receive emails
 * optionally filtered by RID
 * @param  {obj}   args (args.periodEndDate, args.periodLengthDays)
 * @param  {Function} callback
 */
function countUserOptins (args, callback) {
  'use strict';
  var startDateInclusive  = aggregateStartDate(args);
  var endDateInclusive    = aggregateEndDate(args);
  ...
  /*jshint multistr: true */
  query = 'SELECT COUNT(DISTINCT userId) as aggregate FROM mofointegration.vw_wm_metrics_users \
              WHERE subscribeToWebmakerList = 1 \
...
function productAUs (args, callback) {
  ...
  args2.periodLengthDays = args.periodLengthDays || 1;

  countUsersActive (args2, function (err, result) {
    if (err) {
      console.error(err);
      return callback(err);
    }
 ...
 * Description
 * @param  {obj} args
 *             args.snapshotDate (required)
 *             args.periodLengthDays (required)
 * @param  {Function} callback
 */
 ...
 * @param  {Function} callback
 */
function productNewUserOptins (args, callback) {
  'use strict';
  if (!args.snapshotDate) {
    return callback('args.snapshotDate is required');
  }
  ...
  args2.periodEndDate = args.snapshotDate;
  args2.periodLengthDays = args.periodLengthDays || 1;
  countUserOptins (args2, function (err, result) {
    if (err) {
      console.error(err);
      return callback(err);
    }
...
function productRetainedUsersXDays (args, callback) {
  'use strict';
  if (!args.snapshotDate || !args.retentionPeriodInDays) {
    return callback('missing args field in productRetainedUsersXDays()');
  }
  var snapshotDate = args.snapshotDate;
  var retentionPeriodInDays = args.retentionPeriodInDays;
  ...
  usersRetained (userArgs, function (err, result) {
    console.log('retainedUsers:', result);
    callback(null, result);
  });
}
 ...
 * who were active (updatedAt) more than 7 days after account creation
 */
function productRetainedUsers7days (args, callback) {
  'use strict';
  args.retentionPeriodInDays = 7;
  productRetainedUsersXDays(args, function (err, result) {
 ...
 * @param  {Function} callback
 * Reports on the % of users who joined 30-37 days prior to the snapshot date
 * who were active (updatedAt) more than 30 days after account creation
 */
function productRetainedUsers30days (args, callback) {
  'use strict';
  args.retentionPeriodInDays = 30;
  productRetainedUsersXDays(args, function (err, result) {
    result = parseInt(result / 7);
 ...
 * @param  {Function} callback
 * Reports on the % of users who joined 90-97 days prior to the snapshot date
 * who were active (updatedAt) more than 90 days after account creation
 */
function productRetainedUsers90days (args, callback) {
  'use strict';
  args.retentionPeriodInDays = 90;
  productRetainedUsersXDays(args, function (err, result) {
    result = parseInt(result / 7);
...
module.exports = {
  ...
  productNewUserOptins: productNewUserOptins,
  ...
  productRetainedUsers7days: productRetainedUsers7days,
  productRetainedUsers30days: productRetainedUsers30days,
  productRetainedUsers90days: productRetainedUsers90days
};
